...
namespace Run {
    ...
    //        access.Left.SetParent(access);
    //        parent.Scanner.Scan();
    //        access.Right = ParseExpression(access, PrecedenceLevel.Postfix);
    //        if (access.Right == null) {
    //            return null;
    //        }
    //        access.Right.SetParent(access);
    ...
    public class Literal : ValueType {
        ...
        public override void Save(TextWriter writer, Builder builder) {
            if (Token.Type == TokenType.QUOTE) {
                ...
                writer.Write(',');
                writer.Write(Token.Value.Length);
                writer.Write("})");
                return;
            }
            base.Save(writer, builder);
